...
app.post('/login', async (req, res)=>{
    ...
    const [rs] = await db.query('SELECT * FROM member WHERE member_account=?', [req.body.account])
    ...
    const compareResult = await bcrypt.compare(req.body.password, row.member_password);
    if(! compareResult){
        output.error = '帳密錯誤';
        output.code = 402;
        return res.json(output);
    ...
    const {member_id, member_account} = row;
    output.success = true;
    output.info = {member_id, member_account};
    output.token = jwt.sign({member_id, member_account}, process.env.JWT_KEY);
    res.json(output);
});
